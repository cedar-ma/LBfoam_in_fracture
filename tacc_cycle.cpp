...
void readUserDefinedSimulationParameters(std::string xmlInputFileName,
  ...
  document["output"]["grad"].read(param.grad);
...
//  setBoundaryDensity(adLattice, outlet, 0. );
  ...
  boundaryConditionFieldLattice->addVelocityBoundary0P(outlet, fields.lattice, boundary::neumann);
...
//      }
    ...
    if (continueSimulation == true  && iT > param.reload_it && (iT- param.reload_it)% param.grad==0 ) {
      pcout<< "Reading state from bubble pool: " << continueFileName <<std::endl;
      partialloadState(checkpointBlocks, iT, param.saveDynamicContent, continueFileName);
      }
  }
