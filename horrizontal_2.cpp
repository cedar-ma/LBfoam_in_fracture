...
struct SimulationParameters {
  ...
  plint save_it;
  plint restart_it;
  std::string continueFileName;    // File to continue the simulation.
...
void readUserDefinedSimulationParameters(std::string xmlInputFileName,
  ...
  std::string continueFileName = "continue.xml";
...
//  OnLatticeBoundaryCondition2D<T,DESCRIPTOR>* boundaryConditionFieldLattice = createLocalBoundaryCondition2D<T,DESCRIPTOR>();
//  boundaryConditionFieldLattice->addPressureBoundary1N(inlet, fields.lattice);
//  setBoundaryDensity(fields.lattice, inlet,1.0 );
//  boundaryConditionFieldLattice->addPressureBoundary1P(outlet, fields.lattice, boundary::outflow);
// setBoundaryDensity(fields.lattice, outlet,1. - 0.0005*DESCRIPTOR<T>::invCs2 );  
  ...
  for (plint iX=0; iX<param.nx-1; ++iX) {
    for (plint iY=0; iY<param.ny-1; ++iY) {
     ...
     if (param.load_state == true) {

      pcout << "Loading restart files..." << std::endl;
//      pcout << "Reading state of the simulation from file: " << param.continueFileName << std::endl;
//      loadState(checkpointBlocks, param.restart_it, param.saveDynamicContent, param.continueFileName);

      try {
//          std::string iter_name1 = outDir + Latticef + std::to_string(param.restart_it) + ".dat";
//          std::ifstream ifile_1(iter_name1.c_str());
//          ifile_1.read((char *) & fields, sizeof(fields));
//          ifile_1.close();
        loadBinaryBlock(fields.lattice, outDir + Latticef + std::to_string(param.restart_it) + ".dat"); // "lattice_fields.dat"
        loadBinaryBlock(adLattice, outDir + Latticead + std::to_string(param.restart_it) + ".dat"); // "lattice_adLattice.dat"
        loadBinaryBlock(fields.lattice, outDir + Latticef + std::to_string(param.restart_it) + ".dat");
        loadBinaryBlock(fields.volumeFraction, outDir + Latticef + std::to_string(param.restart_it) + "_vof.dat");
        loadBinaryBlock(fields.outsideDensity, outDir + Latticef + std::to_string(param.restart_it) + "_rho.dat");
        loadBinaryBlock(fields.flag, outDir + Latticef + std::to_string(param.restart_it) + "_flag.dat");
        loadBinaryBlock(fields.normal, outDir + Latticef + std::to_string(param.restart_it) + "_normal.dat");
        loadBinaryBlock(fields.rhoBar, outDir + Latticef + std::to_string(param.restart_it) + "_rhoBar.dat");
        loadBinaryBlock(fields.mass, outDir + Latticef + std::to_string(param.restart_it) + "_mass.dat");
        loadBinaryBlock(fields.j, outDir + Latticef + std::to_string(param.restart_it) + "_j.dat");
        loadBinaryBlock(adLattice, outDir + Latticead + std::to_string(param.restart_it) + ".dat");
      } catch (PlbIOException & exception) {
        throw std::runtime_error("Restart files not found.");
      }

      pcout << "Sucessfully found the loaded files!" << std::endl;
...
//          saveBinaryBlock(&fields, outDir + Latticef + std::to_string(iT) + ".dat");
//          for (plint iX=0; iX<param.nx-1; ++iX) {
//              for (plint iY=0; iY<param.ny-1; ++iY) {
//          std::string iter_name1 = outDir + Latticef + std::to_string(iT) + ".dat";
//          std::ofstream ofile_1(iter_name1.c_str());
//          ofile_1.write((char *) & fields, sizeof(fields));
//          ofile_1.close();
...
//          std::string iter_name = outDir + "/bubble_info.dat";
//          plb_ofstream ofile_iter(iter_name.c_str());
//          std::map<plint, BubbleInfo2D>::const_iterator id =
//              bubbleGrowth.getBubbles().begin();
//          for (; id != bubbleGrowth.getBubbles().end(); ++id) {
//              ofile_iter << id->second << std::endl;
//          }
//          ofile_iter.close();
...
//          ofile_iter.close();
    ...
    if (iT % param.statIter == 0 || iT == param.maxIter - 1) {
      ...
      for (; it != bubbleGrowth.getBubbles().end(); ++it) {
        ...
        pcout<<"Volume for frozen ID  "<<it ->first<<" is "<<it->second.getVolume()<<std::endl;
          continue;
        }
        numBubbles++;
