...
def plot_foam_quality_comparison(nx, ny, tmp_folder_list,
                                 geom_file_list,  skip):
    ...
    for i in range(len(foam_list)):
        ...
        titles.append(f'Iterations: {title_iter}')
    ...
    for i in range(0, len(foam_list), skip):
        ...
        # Plotting setup
        x = np.arange(0, fracture.shape[2], 1)
        y = np.arange(0, fracture.shape[1], 1)
        X, Y = np.meshgrid(x, y)
        ...
        plt.title(titles[i], fontsize=fontsize_title)
        plt.xticks([])
        plt.yticks([])
    ...
    # plt.subplots_adjust(wspace=0, hspace=0.4)
        plt.tight_layout()
        plt.savefig(f'{tmp_folder_list}volumeFraction{titles[i][-9:-4]}.png', dpi=500)
        plt.show()
...
sim_folder = '/Users/cedar/Library/CloudStorage/Box-Box/Research_Cedar/LBFOAM/Wyoming wetting meeting 2023/results'

tmp_folder_list = f'{sim_folder}/2D/rough_fracture/restart_weak/'
                   

geom_file_list = f"{sim_folder}/input/gumbo_fracture_horrizontal_dilate.raw"
                  
#%%
calculate_quality_over_sim(nx, ny, tmp_folder=tmp_folder_list[0], geom_file=geom_file_list[0],
                            data_savename='input/smooth_fracture_data.npy', skip=1)
# #
...
quality_data_smooth = np.load('input/smooth_fracture_data.npy')
...
print(smooth_q_60, smooth_ind_60+1)
...
plot_foam_quality_comparison(nx, ny, tmp_folder_list,
                             geom_file_list, skip)
# plt.show()
