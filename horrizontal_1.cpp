...
int main(int argc, char **argv) {
  ...
  Box2D lateral1 (1, 1, 0, param.ny - 1);
...
//  checkpointBlocks.push_back(&fields);
  ...
  for (plint iT = iniIter; iT < param.maxIter; iT++) {
    ...
    if (param.save_sim == true && iT == param.save_it) {
//            pcout << "Saving the state of the simulation at iteration: " << iT << std::endl;
//            saveState(
//                checkpointBlocks, iT, param.saveDynamicContent, param.continueFile,
//                param.checkpointFile, PADDING);
//            pcout << std::endl;
        }
