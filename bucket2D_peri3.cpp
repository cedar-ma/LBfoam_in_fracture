...
void readUserDefinedSimulationParameters(std::string xmlInputFileName,
  ...
  document["output"]["reload_it"].read(param.reload_it);
}
...
int main(int argc, char **argv) {
  ...
  Box2D outlet (1, param.nx - 2, param.ny - 2, param.ny - 2);
  Box2D inlet (1, param.nx - 2, 1, 1);
  Box2D boundaryBox(0,param.nx, 0,param.ny);
...
//  adLattice.periodicity().toggleAll(false);
  ...
  adLattice.periodicity().toggle(1, true);
    ...
    if (param.load_state == true) {
      ...
      pcout << "Starting the sim!" << std::endl;
    }else{
        // Initialization
  ...
  plint iniIter;
    if (param.load_state == true) {
      ...
      if (ifile.is_open()) {
        ifile >> iniIter;
        global::mpi().bCast( & iniIter, 1); // Broadcast so all the processors don't get confused!
        pcout << "Current iteration number: " << iniIter << std::endl;
        ifile.close();
      }
    } else {
      iniIter = 0;
  ...
  for (plint iT = iniIter; iT < param.maxIter; iT++) {
    ...
    applyProcessingFunctional(
      ...
      if (param.save_sim == true && iT > 0 && iT % param.save_it == 0) {
